...
static void add_free_block(void *bp) {
    ...
    for (current = free_listp; GET_ALLOC(HDRP(current)) != 1; current = SUCC_LINK(current)) {
        if (bp <= current) {
            // 맨 앞일 경우
            ...
            // 이후 블럭일 경우
                ...
                SUCC_LINK(PRED_LINK(current)) = bp;
                PRED_LINK(bp) = PRED_LINK(current);
                SUCC_LINK(bp) = current;
                PRED_LINK(current) = bp;
            }
            return;
        }
    }
    
    // 맨 마지막인 프롤로그 블록 직전에 추가하는 경우
    // free list가 비어있는 경우
    if (free_listp == current) {
        SUCC_LINK(bp) = free_listp;
        PRED_LINK(free_listp) = bp;
        PRED_LINK(bp) = NULL;
        free_listp = bp;
